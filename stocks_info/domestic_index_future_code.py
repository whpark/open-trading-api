...
def get_domestic_future_master_dataframe(base_dir):
    ...
    fo_idx_code_zip = zipfile.ZipFile(base_dir + 'fo_idx_code_mts.mst.zip')
    fo_idx_code_zip.extractall(base_dir)
    ...
    df.columns = columns
    df.to_excel(base_dir + 'fo_idx_code_mts.xlsx',index=False)  # 현재 위치에 엑셀파일로 저장
